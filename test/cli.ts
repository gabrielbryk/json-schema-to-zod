...
suite("cli", (test) => {
  ...
  test("runs cli (input only)", (assert) => {
    const { stderr } = spawnSync("tsx", ["src/cli.ts", "-i", "test/all.json"], {
      encoding: "utf8",
    });
    assert(!stderr);
  });
  ...
  test("runs cli (noImport)", (assert) => {
    const { stderr } = spawnSync("tsx", ["src/cli.ts", "-i", "test/all.json", "--noImport"], {
      encoding: "utf8",
    });
    assert(!stderr);
  });
  ...
  test("runs cli (stdin only)", (assert) => {
    const { stderr } = spawnSync("tsx", ["src/cli.ts"], {
      input: '{"type": "any"}',
      encoding: "utf8",
    });
    assert(!stderr);
  ...
  test("runs cli (output)", (assert) => {
    const { stderr } = spawnSync(
      "tsx",
      ["src/cli.ts", "--output", "test/output/output.js", "-i", "test/all.json"],
      {
        encoding: "utf8",
      }
    );
  ...
  test("runs cli (output with depth)", (assert) => {
    const { stderr } = spawnSync(
      "tsx",
      ["src/cli.ts", "--output", "test/output/output.js", "-i", "test/all.json", "-d", "2"],
      {
        encoding: "utf8",
      }
    );
  ...
  test("cli should err with with missing input", (assert) => {
    const { stderr } = spawnSync("tsx", ["src/cli.ts", "--output", "test/output/output.js"], {
      encoding: "utf8",
    });
  ...
  test("cli should err with with bad depth", (assert) => {
    const { stderr } = spawnSync(
      "tsx",
      ["src/cli.ts", "--output", "test/output/output.js", "-i", "test/all.json", "-d", "abc"],
      {
        encoding: "utf8",
      }
    );
  ...
  test("cli should err with with missing depth", (assert) => {
    const { stderr } = spawnSync(
      "tsx",
      ["src/cli.ts", "--output", "test/output/output.js", "-i", "test/all.json", "-d"],
      {
        encoding: "utf8",
      }
    );
  ...
  test("cli accepts type flag when name is provided", (assert) => {
    const { stderr } = spawnSync(
      "tsx",
      ["src/cli.ts", "-i", "test/all.json", "-n", "MySchema", "-t", "MyType"],
      {
        encoding: "utf8",
      }
    );
