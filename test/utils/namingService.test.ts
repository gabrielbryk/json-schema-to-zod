...
describe("namingService", () => {
  test("generates PascalCase names with parent and path", () => {
    ...
    const name = generateNameFromPath({ parentName: "CallTask", path: ["with", "asyncapi"], existingNames: existing });
  ...
  test("uses hook when provided", () => {
    ...
    const existing = new Set<string>();
    const name = generateNameFromPath({ parentName: "Root", path: ["x"], existingNames: existing, nameForPath: hook });
  ...
  test("uses shortest unique tail when title collides", () => {
    ...
    const name = generateNameFromPath({ parentName: "CallTask", path: ["with", "asyncapi", "details"], existingNames: existing });
